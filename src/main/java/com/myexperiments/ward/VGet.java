...
public class VGet {
	 ...
	 * 
	 * will fail if vget extract several sources (for video/audio urls). use
	 * VideoFileInfo.targetFile for multiply sources.
	 * 
	 * @param file
	 */
	...
	public String getContentExt(DownloadInfo dinfo) {
		...
		// for multi file download keep content type and extension. some video can have
		// same extensions for booth
		// audio/video streams
		ct = ct.replaceFirst("/", ".");
	 ...
	 * @param conflict True if we have same file extension for multiply files. File
	 *                 name will be renamed to " (1).ext"
	 */
			...
			while (!done(stop)) {
				try {
					...
					// new targetFile() call
					{
						...
						// safety checks. should it be 'vhs' dependent? does other services return other
						// than
						// "video/audio"?
						...
						// did we meet two similar extensions for audio/video content? conflict == true
						// if so.
						...
						// conflict means we have " (1).ext" download. try add ".content.ext" as
						// extension
